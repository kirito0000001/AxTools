...
原实现每次重启发两封邮件（「异常重启」+「已启动」），且把「服务端本来就没运行、被看门狗首次拉起」也标成异常，导致每次开机或维护后收到大量无意义通知。

### 改动

| 项 | 改动前 | 改动后 |
| --- | --- | --- |
| 单次重启 | 异常重启 + 已启动，共 2 封 | 合并为一条记录，不再单独发「已启动」 |
| 同一轮多个服务 | 每个服务各发 1 封 | 全部合并成 **1 封汇总邮件** |
| 看门狗启动 | 单独发 1 封 | 并入第一轮汇总邮件 |
| 判定 | 只要服务端没在运行就报「异常」 | 只有「本进程周期内见过它正常运行」或「回溯窗口内存在崩溃上下文」才算异常；否则标记为「首次启动」 |

### 异常邮件内容

异常通知现在包含：服务名与端点、类型、触发原因、**崩溃上下文**（崩溃类型 + 错误条件 + 调用栈，取 `CrashContext.runtime-xml`）、Windows 崩溃事件、服务端日志尾部，邮件末尾附**看门狗日志尾部**。因此即使手边没有 AxTools，也能直接从邮件看到死因。

### 事件流

`state\events.ndjson` 中重启事件的 `kind` 由固定值 `restart` 改为动态值：`abnormal`（异常）或 `first-start`（首次启动），便于事后统计真实异常次数。
